...
const Search = () => {
    ...
    const { requests, reset, error, searchGithubUser, isLoading } = React.useContext(GithubContext);
    ...
    const calculateRemainingTime = () => {
        if (reset) {
            const resetDate = new Date(new Date(reset) - new Date());
    ...
    return <section className="section">
        <Wrapper className="section-center">
            ...
            <h5>Limits will refresh after {calculateRemainingTime()}</h5>
        </Wrapper>
    </section>;
};
